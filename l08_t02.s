...
LDR R0, =_Data 		// The address of end of the list
LDR R1, =Data 		// The address of start of the list
STMFD SP!, {R0} 	// Push the end of list onto stack
STMFD SP!, {R1}		// Push the front of list onto stack
BL MinMax
ADD SP, SP, #8 		// Release the parameter memory from the stack
STR R2, [R0, #4] 	// Store the min value in memory
STR R3, [R0,#8] 	// Store the max value in memory
...
BEQ    _MinMax
LDR    R4, [R0], #4 	// Add the next element of the list to R4
CMP    R4, R2 			// Compare the current element against min
MOVLT  R2, R4 			// If the val is < then keep it as new min
CMP    R4, R3 			// Compare the current val with max
MOVGT  R3, R4 //
B      MinMaxLoop
